...
function Account({ currentUser }: AccountProps) {
  ...
  useEffect(() => {  
	const fetchUserInfo = async () => {
	  try {
		const res = await fetch(`https://bensfunnyapi.up.railway.app/:7096/api/User/${userID}`);
  ...
  useEffect(() => {
	const fetchTransactions = async () => {
	  try {
		const res = await fetch(`https://bensfunnyapi.up.railway.app:7096/api/Transactions`);
